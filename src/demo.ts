...
const encodePerfElement = document.getElementById(
  "encode-performance"
) as HTMLSpanElement;
const decodePerfElement = document.getElementById(
  "decode-performance"
) as HTMLSpanElement;
...
function render() {
  ...
  if (blurhash) {
    ...
    encodePerfElement.innerText = `${performance.now() - start} ms`;
    if (pixels) {
  ...
  if (file) {
    ...
    img.onload = function () {
      ...
      setTimeout(() => {
        ...
        decodePerfElement.innerText = `${performance.now() - start} ms`;
